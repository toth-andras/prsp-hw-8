...
###### Примечание
По причинам, которые мне так и не удалось понять, при повторном (или первом) развертывании docker-compose логи могут отображаться неправильно: не будет колонки с уровнем лога, не будет работать фильтрация, новые логи не будут отображаться. Примерно после 5 минут после запуска при повторном открытии grafana в новой вкладке (http://localhost:3000/) логи начинают отображаться корректно
